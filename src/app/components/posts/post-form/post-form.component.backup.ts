...
import { CommonModule } from '@    .file-input {
      padding: 8px;
    }
...
export class PostFormComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  validateFiles(files: File[]): void {
    ...
    // If all validations pass, add files to the selected files array
      ...
      this.postService.createPost(createData).subscribe({
        next: (newPost) => {
          ...
          if (this.selectedFiles.length > 0) {
            this.postService.uploadMedia(newPost.id, this.selectedFiles).subscribe({
              next: () => {
                this.loading = false;
